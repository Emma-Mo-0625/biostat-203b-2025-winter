...
con <- dbConnect(duckdb(), dbdir = "mimic.duckdb", read_only = FALSE)

dbExecute(con, "
  CREATE OR REPLACE TABLE labevents AS 
  SELECT * FROM parquet_scan('labevents_pq/*.parquet')
")
copy_to(con, icustays_tble, "icustays", overwrite = TRUE)
```

```{r}
dbExecute(con, "
  CREATE OR REPLACE TEMP TABLE labevents_filtered AS
  SELECT subject_id, itemid, storetime, valuenum
  FROM labevents
  WHERE itemid IN (50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)
")

# join icustays, and only keep storetime < intime
dbExecute(con, "
  CREATE OR REPLACE TEMP TABLE labevents_icu AS
  SELECT lf.subject_id,
         i.stay_id,
         lf.itemid,
         lf.storetime,
         lf.valuenum,
         i.intime
  FROM labevents_filtered lf
  JOIN icustays i
    ON lf.subject_id = i.subject_id
  WHERE lf.storetime < i.intime
")

# group subject_id, stay_id, itemid, and keep the max storetime
dbExecute(con, "
  CREATE OR REPLACE TEMP TABLE labevents_last AS
  WITH ranked AS (
    SELECT subject_id,
           stay_id,
           itemid,
...
```{r}
labevents_last_df <- dbGetQuery(con, "SELECT * FROM labevents_last")

labevents_wide <- labevents_last_df %>%
  pivot_wider(
...
labevents_tble <- icustays_tble %>%
  left_join(labevents_wide, by = c("subject_id", "stay_id")) %>%
  arrange(subject_id, stay_id)  # order by subject_id, stay_id
```

```{r}
labevents_tble_minimal <- labevents_tble %>%
  select(
    subject_id,
    stay_id,
    bicarbonate,
...
**First generate chartevents_pq using Parquet**
...
**Then use the Parquet data to generate the final table**
...
dbExecute(con, "
  ...
  SELECT * FROM parquet_scan('chartevents_pq/*.parquet')
")
...
```{r}
dbExecute(con, "
  CREATE OR REPLACE TEMP TABLE chartevents_filtered AS
  SELECT subject_id, itemid, storetime, valuenum
  FROM chartevents
...
dbExecute(con, "
  CREATE OR REPLACE TEMP TABLE chartevents_icu AS
  SELECT cf.subject_id,
         i.stay_id,
         cf.itemid,
         cf.storetime,
         AVG(cf.valuenum) AS avg_valuenum,
         i.intime
  FROM chartevents_filtered cf
  JOIN icustays i
    ON cf.subject_id = i.subject_id
  WHERE cf.storetime >= i.intime
  GROUP BY cf.subject_id, i.stay_id, cf.itemid, cf.storetime, i.intime;
")

dbExecute(con, "
  CREATE OR REPLACE TEMP TABLE chartevents_first AS
  WITH ranked AS (
    SELECT subject_id,
           stay_id,
           itemid,
           ...
           ROW_NUMBER() OVER (
             PARTITION BY subject_id, stay_id, itemid
             ORDER BY storetime ASC
           ) AS rn
    FROM chartevents_icu
  )
  SELECT subject_id, stay_id, itemid, avg_valuenum
  FROM ranked
  WHERE rn = 1
")
```
...
```{r}
chartevents_first_df <- dbGetQuery(con, "SELECT * FROM chartevents_first")

chartevents_wide <- chartevents_first_df %>%
  pivot_wider(
    id_cols = c("subject_id", "stay_id"),
    names_from = "itemid",
...
chartevents_tble <- icustays_tble %>%
  left_join(chartevents_wide, by = c("subject_id", "stay_id")) %>%
  arrange(subject_id, stay_id)
```

```{r}
chartevents_tble_minimal <- chartevents_tble %>%
  select(
    subject_id,
    stay_id,
    heart_rate,
...
print(chartevents_tble_minimal)
```

```{r}
rm(chartevents_first_df, chartevents_wide)
dbDisconnect(con)
```
...
mimic_icu_cohort <- icustays_tble %>%
  ...
  # 3) Filter to adult ICU stays (example using anchor_age >= 18 in MIMIC-IV)
  filter(anchor_age >= 18) %>%
  
  # 4) Join the last lab measurements before ICU (one row per ICU stay)
  left_join(labevents_tble, by = c("subject_id", "hadm_id", "stay_id")) %>%
