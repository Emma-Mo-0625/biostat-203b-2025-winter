...
```{r}
# Tuning: Use a Very Narrow/Coarse Grid
narrow_grid <- grid_regular(
  penalty(range = c(0.005, 0.02)),  
  mixture(range = c(0.4, 0.6)),    
  levels = 2
)
narrow_grid
```
...
```{r}
log_reg_narrow_tune |> show_best(metric = "roc_auc")
```

```{r}
best_logit <- log_reg_narrow_tune |>
